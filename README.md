...
스킬은 `$planning-start`, `$planning-check`, `$planning-review`, `$confluence-update-plan`, `$diagram-design` 같은 skill invocation으로 사용합니다.
...
| 플러그인 | 버전 | 목적 | 대표 스킬 | 문서 |
|---|---:|---|---|---|
| `confluence-export-kit` | `0.1.4` | Confluence export 및 local export-index workflow. auth/config 설정, page/space/org/page-with-descendants export, export 후 자동 색인, 로컬 Markdown 재색인을 다룹니다. | `set-config`, `show-config`, `export-page`, `export-page-with-descendant`, `export-space`, `export-org`, `index-export`, `help` | [README](./confluence-export-kit/README.md) |
| `planning-team-kit` | `0.2.1` | 기획자용 기획 문서 workflow. 입력 정리, 선택 점검, 초안 생성, 품질 검토, 수동 Confluence 반영 계획까지 draft-only로 처리합니다. | `help`, `planning-start`, `planning-check`, `planning-draft`, `planning-review`, `confluence-update-plan` | [README](./planning-team-kit/README.md) |

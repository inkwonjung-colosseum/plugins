...
| `planning-kit` | `0.2.8` | 기획 초안(텍스트·파일·디렉터리·URL·이미지)을 정책서·기능설계서 두 본문으로 변환하고 자체 품질을 점검하는 `planning-format`, 산출물을 외부 SSOT 충돌·acceptance criteria·의존 영향 3축으로 검증하는 `planning-review`, 프로젝트 Markdown SSOT corpus 자체를 구조·내용 2축으로 감사하는 `ssot-audit` 세 스킬 구조. 0.2.8부터 `planning-format` 보조 표 헤더는 내부 backlink 없는 clean header만 출력하고, 부모 §/row 추적은 입력 제외 항목의 `구조 변환` 처리 줄에 기록합니다. | `planning-format`, `planning-review`, `ssot-audit` | [README](./planning-kit/README.md) |

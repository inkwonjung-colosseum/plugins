...
| `최신화 후보` | Living standard (`05. 기술문서`, `08. 결정사항 / settled policy`, `12. 운영 가이드`), `design_standard` (Approved Design Reviews in `02. 시스템 디자인`), or `monitoring_standard` (`07. 모니터링` SLO/threshold pages) past its freshness threshold, still current, needs `Refresh required`. Published pages only — never include drafts. Route to `colonova-doc-router-publisher` with the page ID for update. |
| `Pending supersession 후보` | Supersession cycle started but the old page's SUPERSEDED status update did not complete, leaving old and new pages both active (`05`, `08`, `12`, ADR). Route to `colonova-doc-router-publisher` for manual completion. |
| `Archive 후보` | Replacement/current reference/retirement evidence exists. |
| `Manual review 후보` | Draft, whiteboard, body read failure, strong mixed nature, owner check, or link impact. Includes `Discussion closure incomplete`: a decision page was created but the source discussion page was never set to RESOLVED (route to `colonova-doc-router-publisher` to finish the closure). Includes `Manual review (workflow stale)`: a `02` Design Review stuck in `Under Review` past 90 days, needing review restart or `Deprecated` transition, not a content refresh. |

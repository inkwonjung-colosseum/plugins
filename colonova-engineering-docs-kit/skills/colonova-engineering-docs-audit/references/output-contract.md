...
4b. `## 신규 Subfolder 탐지` (produced by SKILL.md workflow step 3b — same section, step number differs by skill convention only) when a live subfolder exists that is not on the known-subfolder list (folder, subfolder title, pageId, page count) — for `colonova-engineering-docs-publish` to update its Subfolder Routing table and folder-id-map Known Subfolders

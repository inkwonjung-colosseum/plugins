#!/usr/bin/env python3
"""Search Confluence pages by keyword, then export matched pages with cme."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search Confluence by keyword and export matched pages with cme."
    )
    parser.add_argument("keyword", help="Keyword or exact phrase to search for")
    parser.add_argument(
        "output_path",
        nargs="?",
...
def build_cql(keyword: str, space_key: str | None = None) -> str:
    escaped = keyword.strip().replace("\\", "\\\\").replace('"', '\\"')
    phrase = f'"\\\"{escaped}\\\""'
    cql = f'type = page and (title ~ {phrase} or text ~ {phrase})'
    if space_key:
...
def search_page_urls(
    site: str, auth_entry: dict, keyword: str, space_key: str | None = None
) -> list[str]:
    cql = build_cql(keyword, space_key)
...
def main() -> int:
    args = parse_args()
    keyword = normalize_nonempty(args.keyword, "Keyword")
    ...
    matched_urls = search_page_urls(site, auth_entry, keyword, args.space_key)

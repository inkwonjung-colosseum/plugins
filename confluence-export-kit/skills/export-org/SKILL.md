...
4. Resolve config through `cme config path`, read it through `cme config list -o json`, then extract the base site from each org URL (`scheme://netloc`) and verify that a configured `auth.confluence` entry with both `username` and `api_token` exists for every site. Do not hardcode `~/.cme/config.yaml`.

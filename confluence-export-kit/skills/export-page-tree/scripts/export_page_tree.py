#!/usr/bin/env python3
"""Export a Confluence page tree after verifying cme auth config."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export a Confluence page and all descendants with confluence-markdown-exporter."
        )
    )
    parser.add_argument("page_url", help="Confluence page URL to export with descendants")
    parser.add_argument(
        "output_path",
        nargs="?",
...
def main() -> int:
    ...
    base_url = extract_base_url(args.page_url)
    ...
    if not args.dry_run:
        result = run_command([cme_path, "pages-with-descendants", args.page_url], env=env)

...
| 0.2.14 | incremental | 0.2.13 | `planning-format`·`planning-review` 결과 우선 출력 + 저장 결과 handoff + `planning-publish-confluence` 저장 폴더 입력 — 신규 public 출력에서 상단 `생성 결과 요약`과 `결정 보드`를 제거한다. `planning-format`은 기본적으로 로컬 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/`에 저장하고 저장 성공 화면에는 `저장 파일`과 `체크해야 할 항목`만 보여준다. 정책서·기능설계서 전문을 화면에 펼치려면 파일 저장하지 않는 `--no-save`를 사용하며, 저장 실패 시에도 결과 손실 방지를 위해 전문을 화면에 출력한다. `planning-review`는 기본 저장 출력의 `## 저장 파일` 경로가 정확히 1개 저장 폴더로 확정되면 canonical 두 파일을 읽고, 결과는 바로 `결론`과 `검토 결과`를 보여준다. `planning-publish-confluence`는 무인자 context-body 발행을 유지하고, 명시적 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더 입력 시 canonical 정책서·기능설계서 두 파일만 읽는다. 미확정·누락·보강점은 하단 `체크해야 할 항목`으로 모으며, 기존 검증·출처·입력 제외 상세는 `출처/누락 요약`, `검토 근거 요약`, 조건부 `상세 추적`으로 재배치한다 |

## 호환성 요약

- **0.1.x → 0.2.0**: **breaking** (스킬 이름 변경 + 분할). 호출자 마이그레이션 필요 — `formalize` → `planning-format` + 선택적 `planning-review`.
- **그 외**: incremental, 출력 markdown micro-breaking 가능 (다운스트림 파서 영향 시 각 PRD 호환성 절 참조). 0.2.14 신규 `planning-format` 기본 저장 성공 출력은 첫 H2가 `## 저장 파일`이고 top-level H2는 `## 저장 파일`, `## 체크해야 할 항목`뿐이다. `planning-format --no-save`와 저장 실패 fallback 출력은 첫 H2가 `## 정책서`, 신규 `planning-review` 출력은 첫 H2가 `## 결론`이다. parser는 `## 저장 파일`, `## 결론`, `## 체크해야 할 항목`, `## 출처/누락 요약`, `## 검토 결과`, `## 검토 근거 요약`, `## 상세 추적`, `## 저장 실패 상세`를 정책서·기능설계서 본문 종료 boundary 또는 review report boundary로 인식해야 한다. `## 결정 보드`는 0.2.14 신규 출력과 parser 목표에서 제외한다. 0.2.14부터 `planning-format`은 저장 기본값이며 `--no-save`만 화면 only 동작이다. 기존 `--save`는 같은 저장 동작으로 허용한다. `planning-review`는 직전 기본 저장 출력의 `## 저장 파일`이 정확히 1개 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더를 가리키면 canonical 두 파일을 읽을 수 있다. `planning-publish-confluence`는 명시적 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더 입력이 있을 때만 저장 파일을 읽고, URL·임의 `.md` 추론·여러 폴더 자동 선택은 거부한다. 최신 PRD가 release target인 경우 해당 PRD의 롤아웃 계획 완료 전까지 runtime 현재 계약으로 보지 않는다.
...
- [`prd-0.2.14.md`](./prd-0.2.14.md) — `planning-format`은 기본적으로 로컬 저장 후 저장 성공 화면에는 `저장 파일`과 `체크해야 할 항목`만 보여주며, 저장 실패 시에는 정책서·기능설계서 전문을 화면에 출력한다. `planning-review`는 직전 저장 출력의 `## 저장 파일` 경로가 정확히 1개 저장 폴더로 확정되면 canonical 두 파일을 읽고, 결과는 결론과 검토 결과를 먼저 보여준다. `planning-publish-confluence`는 기존 context-body 발행과 명시적 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더 발행을 모두 지원하되, 저장 폴더에서는 canonical 정책서·기능설계서 두 파일만 읽는다. 신규 기본 저장 성공 출력에서는 `planning-format`의 첫 H2가 `## 저장 파일`, `planning-format --no-save`와 저장 실패 fallback에서는 첫 H2가 `## 정책서`, `planning-review`의 첫 H2가 `## 결론`이다. 하단 `체크해야 할 항목`에 `결정 필요`, `문서 보강 필요`, `출처/누락 참고` 또는 review 후속 행동을 모은다. `## 결정 보드`는 0.2.14 신규 출력과 parser 목표에서 제외한다.
...
- [`fixtures/prd-0.2.14-fixtures.yml`](./fixtures/prd-0.2.14-fixtures.yml) — 기본 저장 성공, 저장 실패 fallback, `--no-save`, `planning-review` 저장 파일 handoff, 저장 실패 fallback의 review/publish 소비, 0.2.14 parser boundary, `planning-publish-confluence` `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더 입력 기대값.

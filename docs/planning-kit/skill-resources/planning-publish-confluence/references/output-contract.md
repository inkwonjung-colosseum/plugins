...
- 결과: 발행 취소
- 이유: 지원하지 않는 입력입니다. 0.2.14는 현재 context memory 안의 두 본문 또는 명시적 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` 저장 폴더만 발행 대상으로 사용합니다.
- Confluence 변경: 없음

## 금지 입력

- URL, 임의 `.md` 파일, 중첩/여러 저장 폴더, 옵션 인자는 지원하지 않습니다.
```
...
부족 항목은 최대 5개 bullet로 제한한다. URL이나 임의 `.md` 파일을 달라는 재실행 안내는 출력하지 않는다. 명시적 저장 폴더 안내가 필요하면 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` direct child 1개만 지원한다고 쓴다.

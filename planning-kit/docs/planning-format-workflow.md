...
flowchart TD
  ...
  M --> DB{결정 보드 필요?<br/>사용자 확인 / 릴리즈 차단}
  DB --> N{--save?}
...
flowchart TD
  ...
  KEEP --> NSR
  NSR -->|예| OUT["readable 본문 + 요약 출력<br/>차단 입력이면 결정 보드 가능<br/>검증 피드백 생략"]
...
flowchart LR
  ...
  H --> DB{결정 보드 필요?}
  DB -->|예| BOARD["## 결정 보드<br/>첫 화면 요약 / D* / A* / 차단"]
...
블록 순서: 사용자 확인 필요 항목 또는 릴리즈 차단 항목이 있으면 **헤더 요약 → 결정 보드 → 정책서 → 기능설계서 → 검증 피드백 → 출처 요약 → 입력 제외 요약 → 상세 추적**. 둘 다 없으면 `## 결정 보드`를 생략하고 0.2.9 순서인 **헤더 요약 → 정책서 → 기능설계서 → 검증 피드백 → 출처 요약 → 입력 제외 요약 → 상세 추적**을 유지한다. `## 검증 피드백`은 `--no-self-review`에서 생략하고, `## 상세 추적`은 실패/본문 미사용/high-signal 제외 항목, 결정 보드 연결 맵 등 조건 충족 시 출력한다.

결정 보드는 canonical 본문이 아니라 사용자가 지금 결정·수정·검증할 항목을 앞세우는 projection이다. 상단에는 `범례`, `읽는 순서`, `첫 화면 요약`, `지금 결정해야 할 항목`, `릴리즈 차단 항목`, `바로 수정할 문서 작업`이 들어가며, 원시 F* ID와 connector 세부 상태는 하단 `## 검증 피드백` 또는 `## 상세 추적`으로 내린다.

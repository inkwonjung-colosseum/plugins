...
- Confluence `[AI]` 영역의 SSOT 문서를 로컬 폴더로 동기화한다.
...
- 로컬 SSOT 폴더를 기준으로 `planning-format`과 `planning-review`를 실행한다.
...
```mermaid
flowchart LR
  A["Confluence [AI]"] --> B["Confluence Export Desktop"]
  B --> C["로컬 SSOT 출력 폴더"]
  ...
  E --> F["planning-format / planning-review"]
  J --> F
  F --> G["정책서 / 기능설계서<br/>SSOT 충돌 / AC / 의존 영향 결과"]
```

| 구성 요소 | 역할 | 필수 여부 |
|---|---|---|
| Confluence Export Desktop | Confluence `[AI]` 문서를 로컬 Markdown SSOT 폴더로 가져오는 Windows 앱 | 필수 |
...
| planning-kit | 기획 초안을 정책서/기능설계서로 변환하고 review하는 플러그인 | 필수 |
...
| 항목 | 링크 | 비고 |
|---|---|---|
| Confluence Export Desktop | https://github.com/inkwonjung-colosseum/confluence-export-desktop/releases/tag/v0.1.0 | `[AI]` SSOT export 앱 |
...
| Confluence API token | https://id.atlassian.com/manage-profile/security/api-tokens | `[AI]` 영역을 읽을 수 있는 Confluence 계정으로 발급 |
...
Confluence Export Desktop은 Confluence `[AI]` 영역의 SSOT 문서를 로컬 Markdown 폴더로 가져오는 Windows 앱입니다.

이 앱은 사용자 PC에 Python을 별도로 설치하지 않고 실행되며, API token은 Windows Credential Manager에 저장합니다. 앱 자체에 예약 실행과 트레이/백그라운드 스케줄러 런타임이 포함되어 있어 `[AI]` SSOT를 주기적으로 내려받는 용도로 사용합니다.
...
| 대상 범위 | Confluence `[AI]` 영역 또는 `[AI]` 하위 space/page |
...
- `.md` 파일이 생성되어 있다.
- 정책서/기능설계서 또는 Confluence `[AI]` 문서 제목이 파일명이나 본문에 보인다.
...
Confluence `[AI]` SSOT는 주기적으로 최신 상태로 내려받아야 합니다.
...
- Confluence `[AI]`, Confluence `[Origin]`, Jira를 열람할 수 있는 계정으로 Atlassian 커넥터가 연결되어 있다.
...
Cowork에서 사용할 때는 입력창에서 `/`를 입력하거나 `+` 버튼을 눌러 `planning-format`, `planning-review` 스킬을 선택합니다.
...
2. SSOT 출력 폴더에 최신 [AI] SSOT가 있는지 확인한다.
...
8. 발견 항목을 수정하거나 기획팀/실무 리뷰 안건으로 남긴다.
9. 정리된 결과를 Confluence [AI]에 업데이트한다.
```
...
| Confluence export가 실패함 | API token, Confluence 권한, 네트워크 | 토큰 재발급, `[AI]` 영역 읽기 권한 확인 |
...
- SSOT 출력 폴더에 Confluence `[AI]` Markdown 문서가 내려와 있다.

...
같은 발견이 여러 축에 걸치면 한 번만 기록합니다. 우선순위는 `R1 > R3 > R2`입니다. R1은 기존 기준과의 직접 충돌이므로 가장 강한 신호로 봅니다. 기본 보고서는 축별 원시 목록보다 `판정`, `검증 신뢰도`, `최우선 수정 항목`, `작업 백로그`를 먼저 보여줍니다.
...
| 대표 출력 | 검증 피드백, 입력 제외 요약 | 판정, 검증 신뢰도, 최우선 수정 항목, 작업 백로그, 상세 추적 |
...
| 최우선 수정 항목 | P0/P1로 먼저 처리할 발견 |
| 작업 백로그 | 문서 수정, 정책 결정, SSOT 보강, 외부 인터페이스 작업 |
...
4. planning-format이 정책서, 기능설계서, 검증 피드백, 입력 제외 요약을 만든다.
5. 실행 담당과 기획팀이 [TBD], 검증 피드백, 입력 제외 요약을 확인한다.

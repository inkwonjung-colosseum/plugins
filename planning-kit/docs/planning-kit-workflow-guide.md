...
사용자 확인 필요 항목이나 릴리즈 차단 항목이 있으면 0.2.10부터 본문 앞에 `## 결정 보드`가 먼저 나옵니다. 결정 보드는 canonical 정책서/기능설계서가 아니라, 오늘 결정할 D*, 바로 수정할 A*, 릴리즈 차단 사유를 첫 화면에서 보이게 하는 읽기용 projection입니다.
...
같은 발견이 여러 축에 걸치면 한 번만 기록합니다. 우선순위는 `R1 > R3 > R2`입니다. R1은 기존 기준과의 직접 충돌이므로 가장 강한 신호로 봅니다. 0.2.10 기본 보고서는 축별 원시 목록보다 `## 결정 보드`를 먼저 보여줍니다. 같은 원인의 R2/R3 또는 R1/R3 finding은 하나의 D*/A*로 묶고, 원시 R* ID는 하단 상세 추적의 연결 맵에서 확인합니다.
...
flowchart LR
  ...
  E["SSOT token 폴더 기준 문서"] --> F["SSOT 매칭"]
  F --> G["SSOT 자료 수집"]
  G --> D
...
| SSOT 자료 수집 | 기존 기준과 비교하기 위해 SSOT token 폴더 Markdown과 그 안의 링크를 확인 | `## 검증 범위와 한계`, `## 출처 요약`, 조건부 `## 상세 추적` |
...
- `planning/**`과 `.planning-kit/**`은 review 대상 입력은 될 수 있지만 SSOT 근거는 될 수 없다.
...
| 대표 출력 | 결정 보드(필요 시), 검증 피드백, 입력 제외 요약 | 결정 보드, 판정, 검증 신뢰도, legacy 요약, 상세 추적 |
...
```mermaid
flowchart LR
  A["SSOT token 폴더 Markdown"] --> B["placeholder 판정<br/>token 밖/초안 영역 제외"]
...
- 기준 문서가 SSOT token 폴더 밖에 있지는 않은가
...
| 결정 보드 | 사용자 확인·릴리즈 차단이 있을 때 오늘 결정할 D*, 바로 수정할 A*, 차단 사유 |
...
| 결정 보드 | 결론, 오늘 결정, 릴리즈 차단, 바로 수정, 결정 후 반영 |
| 최우선 수정 항목 | 0.2.x 호환용 요약. 실제 판단 순서는 결정 보드를 우선 |
| 작업 백로그 | 0.2.x 호환용 요약. 문서 수정, 정책 결정, SSOT 보강, 외부 인터페이스 작업 |
| 검증 범위와 한계 | readable projection, placeholder, fetch 실패, 비활성 축 한계 |
...
| 출력 | 읽는 법 |
|---|---|
| SSOT 인벤토리 | 실질 본문과 placeholder 분포 |
| 제외 요약 | SSOT token 밖, 생성 초안 영역, 내부 plugin/skill 문서 제외 |
...
2. 기존 SSOT token 폴더가 없거나 placeholder가 많으면 ssot-audit로 backlog를 먼저 확인한다.
3. planning-kit 실행 담당이 Origin 링크를 planning-format에 입력한다.
4. planning-format이 필요 시 결정 보드, 정책서, 기능설계서, 검증 피드백, 입력 제외 요약을 만든다.
5. 실행 담당과 기획팀이 결정 보드의 D*/A*/릴리즈 차단을 먼저 확인한 뒤 [TBD], 검증 피드백, 입력 제외 요약을 확인한다.

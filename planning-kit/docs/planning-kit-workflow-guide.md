...
flowchart LR
  ...
  E --> F["정책서·기능설계서 변환"]
  F --> G["입력 제외 요약 생성"]
  G --> H["F1~F6 자체 검증<br/>검증 피드백"]
  H --> RS["생성 결과 요약<br/>7개 필수 라벨"]
  RS --> DB{"결정 보드 필요?"}
  DB --> I{"--save?"}
  I -->|"예"| J["planning/ 저장"]
  I -->|"아니오"| K["화면 출력"]
...
0.2.12부터 본문 앞에는 항상 `## 생성 결과 요약`이 먼저 나옵니다. 사용자 확인 필요 항목이나 출시 전 해결 필요 항목이 있으면 그 다음에 `## 결정 보드`가 나옵니다. 결정 보드는 canonical 정책서/기능설계서가 아니라, 오늘 결정할 항목, 바로 수정할 작업, 출시 전 해결 필요 사유를 첫 화면에서 보이게 하는 읽기용 projection입니다.
...
`planning-format`은 변환 직후 자체 품질 검증을 수행합니다. 이 검증은 외부 SSOT를 보지 않고, 산출물 내부 품질과 입력 반영 상태를 봅니다. 0.2.9부터 의미 변경이 필요한 발견은 본문에 조용히 반영하지 않고 `## 검증 피드백`에 먼저 노출합니다.
...
flowchart LR
  ...
  B -->|"파일 / 붙여 넣은 Markdown 원문"| F["본문 분리"]
...
| 붙여 넣은 Markdown 원문 | 두 본문이 들어 있는 markdown을 직접 리뷰 |
...
같은 발견이 여러 축에 걸치면 한 번만 기록합니다. 우선순위는 `R1 > R3 > R2`입니다. R1은 기존 기준과의 직접 충돌이므로 가장 강한 신호로 봅니다. 0.2.12 기본 보고서는 축별 원시 목록보다 `## 결정 보드`를 먼저 보여줍니다. 같은 원인의 R2/R3 또는 R1/R3 finding은 하나의 결정/작업 항목으로 묶고, 원시 발견 ID는 하단 상세 추적의 연결 맵에서 확인합니다.
...
| 구분 | 목적 | 출력 위치 |
|---|---|---|
| 입력 자료 수집 | review 대상 정책서·기능설계서 본문을 만들기 위해 입력 URL과 이미지에서 내용을 수집 | `## 출처 요약`, 조건부 `## 상세 추적` |
| 기준 문서 자료 수집 | 기존 기준과 비교하기 위해 `SSOT` 표시 폴더 Markdown과 그 안의 링크를 확인 | `## 검증 범위와 한계`, `## 출처 요약`, 조건부 `## 상세 추적` |

핵심 동작:

- 입력으로 준 URL 자체가 곧 기존 기준 문서가 되는 것은 아니다.
- 입력 자료 수집과 기준 문서 자료 수집은 목적과 출처 표가 분리된다.
- 같은 URL이 입력과 기준 문서 양쪽에 등장해도 역할이 다르면 각각 기록될 수 있다.
- `planning/**`과 `.planning-kit/**`은 review 대상 입력은 될 수 있지만 기준 문서 근거는 될 수 없다.
...
| 대표 출력 | 생성 결과 요약, 결정 보드(필요 시), 검증 피드백, 입력 제외 요약 | 결정 보드, 판정, 검증 신뢰도, 호환용 최소 표시, 상세 추적 |
...
```mermaid
flowchart LR
  A["SSOT 표시 폴더 Markdown"] --> B["본문 없는 문서 판정<br/>표시 밖/초안 영역 제외"]
  B --> C["외부 링크 follow<br/>접근 재시도"]
  C --> D["SSOT map 생성"]
  D --> E["구조 품질"]
  D --> F["내용 품질"]
  E --> G["개선 backlog"]
...
- archive, draft 문서를 최신 기준처럼 참조하는가
- 기준 문서가 `SSOT` 표시 폴더 밖에 있지는 않은가
...
| 출력 | 읽는 법 |
|---|---|
| 생성 결과 요약 | 문서 생성 결과, 저장, 검증 상태, 확인 필요, 출처 영향을 먼저 확인 |
| 정책서 | 기획팀이 결정해야 할 기준과 규칙 |
| 기능설계서 | 개발/디자인/QA가 구현·검증해야 할 동작 |
| 결정 보드 | 사용자 확인·출시 전 해결 필요가 있을 때 오늘 결정할 항목, 바로 수정할 작업, 차단 사유 |
| 검증 피드백 | 본문 자동 수정 전 사용자 확인이 필요한 발견 |
| 출처 요약 | 어떤 입력과 링크가 근거로 합류했는지 |
...
| 검증 신뢰도 | 충분 / 제한적 / 낮음과 그 이유 |
| 결정 보드 | 결론, 오늘 결정, 출시 전 해결 필요, 바로 수정, 다음 액션 |
| 최우선 수정 항목 | 0.2.x 호환용 최소 표시. 실제 판단 순서는 결정 보드를 우선 |
| 작업 백로그 | 0.2.x 호환용 최소 표시. 문서 수정, 정책 결정, SSOT 보강, 외부 인터페이스 작업 |
...
| 출력 | 읽는 법 |
|---|---|
| SSOT 인벤토리 | 실질 본문과 본문 없는 문서 분포 |
| 제외 요약 | `SSOT` 표시 밖, 생성 초안 영역, 내부 plugin/skill 문서 제외 |
| 상세 추적 | full 후보표, 제외표, 외부 링크 follow 결과 |
...
2. 기존 `SSOT` 표시 폴더가 없거나 본문 없는 문서가 많으면 ssot-audit로 backlog를 먼저 확인한다.

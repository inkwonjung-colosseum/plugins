...
> **planning-kit 이식 노트** — 이 파일은 [epoko77-ai/im-not-ai](https://github.com/epoko77-ai/im-not-ai)에서 가져온 원본 분류 SSOT다. 본문에 등장하는 `see_scholarship: scholarship.md#…` 출처 anchor, `metrics_v2.py`·`_workspace/`·candidate 풀·에이전트(detector·rewriter 등) 언급은 원본 저장소의 보조 자산·변경 이력 메타이며 **이 포트에는 동봉되지 않는다**. 학술 출처 전문이 필요하면 원본 저장소를 참조한다. 패턴 표(정의·심각도·처방)와 `quick-rules.md` 매핑 자체는 그대로 유효하다.

...
필수 위치 인자: 텍스트, 파일 경로, 디렉터리 경로, 이미지 경로, 또는 하나 이상의 `https?://` URL. 옵션은 없다. fetch·이미지 해석·자가검증·저장은 항상 수행한다.
...
3. 모든 모드에서 링크/이미지 수집.
4. URL 본문을 BFS로 fetch.
5. 이미지 해석.
6. 소스 병합.
7. 두 템플릿으로 정책서·기능설계서 생성 (`conversion.md`).
8. 제외 추적 (`conversion.md`).
9. 자가검증 (`self-review.md`).
10. 저장.
11. 출력 계약 방출.

## URL 수집 / connector

- dequeue된 모든 `https?://` URL을 먼저 직접 fetch.
...
항상 저장한다. 저장 위치 `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/`. 충돌은 `--2`, `--3` 등으로 해소.
...
- 저장: `planning/[안전기능명]--YYYY-MM-DD-HHMMSS/` / `실패 - 사유`

저장 성공: 헤더 → `## 저장 파일` → `## 체크해야 할 항목`만 보여준다. 본문 미출력.

저장 실패: 정책서·기능설계서 본문 + `## 저장 실패 상세`(실패 단계·대상 경로·남은 파일/폴더).

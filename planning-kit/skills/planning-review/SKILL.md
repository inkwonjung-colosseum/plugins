...
description: "planning-format 산출물(정책서·기능설계서 두 본문)을 외부 기준 문서 묶음 충돌·acceptance criteria 검증가능성·의존 영향 분석 3축으로 점검할 때 사용한다. 직전 turn의 planning-format 출력, 디렉터리·파일 경로, 붙여 넣은 Markdown 원문, 1개 이상의 URL 입력을 모두 받는다. 1개 파일 입력은 same-folder companion read로 같은 폴더 sibling 파일을 non-recursive 수집해 정책서·기능설계서 쌍을 식별한다."
argument-hint: "[<URL... | 정책서·기능설계서 경로 | 디렉터리 | 붙여 넣은 Markdown 원문>] [--ssot-include <glob>] [--axes <list>] [--no-input-fetch] [--no-input-image] [--no-ssot-fetch] [--no-ssot-image]"
---
...
2. 0.2.10~0.2.12 unsaved 화면 출력은 readable projection으로 취급한다. `## 생성 결과 요약`과 `## 결정 보드`는 report metadata이며 정책서·기능설계서 본문이 아니다.
...
4. backtick/tilde fenced code block 안, blockquote 안, 리스트 하위의 `## 생성 결과 요약` 또는 `## 결정 보드` 문자열은 wrapper heading으로 보지 않는다. 닫히지 않은 fenced code block이 있으면 이후 heading은 wrapper로 인정하지 않고 `readable projection boundary ambiguous` warning을 남긴다.
5. `planning-format` 화면 출력에서는 첫 `## 정책서`보다 앞의 정상 위치 `## 생성 결과 요약`을 report metadata로 취급한다. 같은 metadata 영역에 두 번째 `## 생성 결과 요약`이 있거나 정상 위치 밖에 나타나면 해당 line부터 다음 인정된 wrapper H2 직전 또는 EOF까지 body에서 제외하고 warning 문자열은 정확히 `readable projection boundary ambiguous`로 남긴다.
6. `planning-format` 화면 출력에서는 첫 `## 정책서`보다 앞의 첫 번째 `## 결정 보드`만 report metadata로 취급한다. `planning-review` 화면 출력에서는 첫 `## 결론`보다 앞의 첫 번째 `## 결정 보드`만 review report metadata로 취급한다.
7. 같은 metadata 영역에 추가 `## 결정 보드`가 있거나 metadata 위치가 아닌 곳에 `## 결정 보드`가 나타나면 invalid wrapper sentinel로 감지한다. 해당 line부터 다음 인정된 wrapper H2 직전 또는 EOF까지 body에서 제외하고 warning 문자열은 정확히 `readable projection boundary ambiguous`로 남긴다.
8. 정책서 본문은 첫 `## 정책서` wrapper heading 다음 줄에서 시작한다.
9. 기능설계서 본문은 첫 `## 기능설계서` wrapper heading 다음 줄에서 시작한다.
10. 정책서·기능설계서 본문 종료 경계는 0.2.9와 동일하게 다음 wrapper heading 중 먼저 등장하는 항목이다: `## 기능설계서`, `## 검증 피드백`, `## 출처 요약`, `## 입력 제외 요약`, `## 상세 추적`, `## 결론`, `## 최우선 수정 항목`, `## 작업 백로그`, EOF. `## 생성 결과 요약`과 `## 결정 보드`는 정상 본문 경계로 사용하지 않고, misplaced/duplicate sentinel일 때만 제외 range를 만든다.
...
작업 백로그는 발견을 그대로 나열하기 전에 수정 작업 단위로 묶는다. 항목 ID는 `A1`, `A2` 형식이다. 0.2.10부터 외부 결정 필요 항목은 `D*`, 독립 출시 전 해결 필요 추적은 `T*`, 원시 발견은 `R*`로 분리한다.
...
- `T*` 카드는 `위치`, `차단 이유`, `해소 조건`, `연결`을 포함한다. 차단 사유가 D*/A*에 충분히 표시되면 별도 T*를 만들지 않고 D*/A*에 `[출시 전 해결]` badge를 붙이며 `출시 전 해결 필요 항목` subsection에 한 줄 사유를 남긴다.
...
- 여러 P2 동기화 권고가 같은 운영 워크플로로 해소되면 하나의 A*로 묶고, 결정 보드에는 출시 전 해결 필요 항목과 직접 연결될 때만 올린다.
- 서로 다른 결정 필요자, 서로 다른 반영 위치, 서로 다른 완료 조건 중 하나 이상을 가진 finding은 병합하지 않는다.

특수 상황:

- P2-only review에서 출시 전 해결 필요 항목이 없으면 결정 보드의 결정/차단 subsection은 `없음`으로 표시하고, P2 작업은 top-level `## 작업 백로그` 호환용 요약과 `## 발견 요약`에 압축한다.
- `검증 신뢰도: 낮음`의 원인이 기준 문서 묶음 0건 또는 모두 본문 없는 문서뿐이고 R2/R3 finding이 없으면 D*를 만들지 않는다. SSOT 보강 A*는 top-level `## 작업 백로그` 호환용 요약에만 표시하고 판정은 `비교 불가`를 유지한다.
...
2. `비교 불가`: 요청한 핵심 축이 증거 부족으로 판단 불가다. 예: `--axes ssot`인데 기준 문서 묶음이 모두 본문 없는 문서.
...
신뢰도는 활성 축 기준으로 판단한다. 예를 들어 `--axes ac`만 활성인 경우 기준 문서 묶음 부재는 신뢰도 산정에 영향을 주지 않는다. `낮음`이고 P0/P1/P2 발견이 0건이면 최종 판정은 `비교 불가`다. `낮음`이더라도 P0/P1 발견이 있으면 `수정 필요`다.
...
[최소 표시. 예: 작업 1 (A1) - 결정 보드 참조. 없으면 `없음`]
...
[조건 충족 시 full 입력 출처표 / full 기준 문서 출처표 / 축별 원시 발견 목록]
````
...
- `## 결정 보드`는 `첫 화면 요약`, 항목 목록, 필요 시 상세, 범례를 포함한다. `읽는 순서`는 `오늘 결정 → 출시 전 해결 필요 → 바로 수정 → 상세 근거`로 표시한다.
- `첫 화면 요약`은 최대 5줄이며 각 bullet은 `결론`, `오늘 결정`, `출시 전 해결 필요`, `바로 수정`, `다음 액션` 중 하나로 시작한다.
- 첫 화면 bullet은 `D1`, `A1`, `T1`, `R1-1`, `P0` 같은 원시 ID로 시작하지 않는다.
- 첫 화면과 항목 제목은 `결정 1 (D1)`, `작업 1 (A1)`, `출시 전 해결 1 (T1)`, `차단 (P0)`, `중요 (P1)`, `권고 (P2)`처럼 자연어 라벨을 먼저 쓴다.
- 항목이 없는 결정 보드 subsection은 heading 아래 본문을 정확히 `없음`으로 둔다. `### 지금 결정해야 할 항목: 없음`처럼 heading과 값을 합치지 않는다.
- `결론`은 발견 상세보다 먼저 온다.
- `## 발견 요약`, `## 검증 범위와 한계`, `## 출처 요약`은 legacy 호환 heading보다 먼저 온다.
- top-level `## 최우선 수정 항목`과 `## 작업 백로그` heading 이름은 정확히 유지한다. 괄호나 접미사를 붙이지 않는다.
- 두 legacy 섹션 첫 줄에는 `요약: 실제 우선순위와 실행 순서는 위 결정 보드를 기준으로 확인하세요.`를 붙인다.
- legacy 섹션의 같은 ID는 `작업 1 (A1) - 결정 보드 참조`처럼 1줄로 표시한다.
- legacy 섹션은 각각 최대 5행을 기본으로 한다.
- legacy 섹션 안에 `선택지:`, `검증 방법:`, `완료 조건:`, `Given `, `When `, `Then `, 원시 `R\d+-\d+`를 넣지 않는다.
- 새 판단이나 새 항목은 legacy 섹션에서 처음 만들지 않는다.
...
- `## 상세 추적` 내부에서 `### 결정 보드 연결 맵`은 축별 원시 발견 목록과 입력/기준 문서 출처표보다 먼저 둔다.
- R3 발견·권고 항목이 입력 제외 섹션 보조 신호로 만들어진 경우 `근거` 줄에 입력 제외 섹션 cross-reference 표시.
- 상단 결정 보드에는 connector 세부 status, full source URL table, full 기준 문서 표, 원시 finding 전문을 출력하지 않는다.
- 상단 결정 보드에는 `SSOT corpus`, `fetch`, `deps`, `AC Blocker`를 단독 표기로 쓰지 않고, 각각 `기준 문서 묶음`, `본문 가져오기`, `영향 분석`, `구현/QA 차단`으로 표시한다.
...
최종 응답을 제출하기 직전에 전체 사용자-facing 출력(헤더 요약, 결정 보드, 결론, 발견 요약, 검증 범위와 한계, 출처 요약, legacy 호환 섹션, 상세 추적)을 한 번 더 스캔한다. 원문 직접 인용 안에 실제로 들어 있는 문자를 제외하고 아래 legacy/내부 표현이 남아 있으면 반드시 치환한 뒤 응답한다.

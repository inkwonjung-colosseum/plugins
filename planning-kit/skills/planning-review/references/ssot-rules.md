...
`planning-review`의 SSOT 충돌 점검 축(R1). 0.2.9부터 SSOT corpus는 프로젝트 전체 Markdown이 아니라 폴더명에 독립 `SSOT` token이 있는 하위 폴더 안의 Markdown으로 제한한다. `planning/**`과 `.planning-kit/**`은 생성 초안 영역이므로 항상 제외한다.

## 용어

- **SSOT 기준 문서 묶음**: 폴더명에 독립 `SSOT` token이 있는 하위 폴더 안의 Markdown 후보 중 키워드 매칭과 `--ssot-include` narrowing을 통과한 corpus.
- **input collection**: `planning-review`가 review 대상 본문을 만들기 위해 URL·파일·디렉터리·텍스트·이미지를 수집하는 단계. 입력 URL source 자체는 SSOT corpus가 아니다.
...
2. `planning/**`과 `.planning-kit/**`은 항상 제외한다. 이 제외가 SSOT token 폴더 허용보다 우선한다. 예: `planning/[SSOT]/policy.md`도 제외한다.
...
6. `--ssot-include <glob>`이 있으면 위 후보 안에서만 narrowing한다. glob이 SSOT 폴더 밖만 가리키면 결과는 0건이고 `검증 범위와 한계` 및 `## 상세 추적`에 `명시 include가 SSOT 폴더 경계 밖이라 제외됨`을 남긴다.
...
- `ssot` 축이 활성이고 SSOT 기준 문서 묶음이 0건 또는 모두 placeholder이면 검증 신뢰도는 `낮음`이다.
...
- SSOT token 후보 0건 또는 keyword 매칭 0건 → R1 결과 `비교 대상 본문 없음`.
- 프로젝트 전체 Markdown으로 fallback하지 않는다.
- `## 검증 범위와 한계`에 SSOT token 폴더 경계와 제외 요약을 남긴다.
- `## 상세 추적`에는 후보/제외/placeholder 경로 요약 또는 include 경계 밖 사유를 남긴다.
...
0.2.9 기본 출력은 full `## SSOT 출처` 표를 상단에 두지 않는다. 다음 조건이면 `## 상세 추적` 안에 `### SSOT 출처`를 출력한다.

- link follow가 1건 이상 진행됨.
- SSOT 기준 문서 묶음 0건.
- 매칭 SSOT가 모두 placeholder.
- `--ssot-include`가 SSOT 폴더 경계 밖만 가리켜 후보가 0건.
- SSOT link follow 실패, 인증 실패, 본문 미사용 출처가 1건 이상.
...
| 2 | 매칭 *.md | Product SSOT/todo.md | - | placeholder | X |
...
실패 행도 list에 포함한다. 실패 0건이면 표 아래 `모든 corpus 외부 링크 처리 성공` 한 줄을 둘 수 있다.

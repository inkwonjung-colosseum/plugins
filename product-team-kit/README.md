...
`plan-format`은 3-step 변환 흐름이다 (config strict-exit → Gate First → dispatch 후 단일 패스 작성·자체 검증·저장). 입력 판정, lazy read 순서, 분류 기준, marker 규칙은 `skills/plan-format/SKILL.md`에서 단일 소스로 따른다. 저장 위치와 안전기능명, 출력 템플릿은 필요한 분기에서만 `skills/plan-format/references/storage-contract.md`와 `skills/plan-format/references/output-contract.md`를 읽어 따른다. `plan-review`의 SSOT corpus 선택, main A축 점검, B/C/D worker 점검 기준, 합성 규칙, 출력 템플릿은 필요한 시점에 `skills/plan-review/references/review-rules.md`와 `skills/plan-review/references/output-format.md`를 읽어 따른다.

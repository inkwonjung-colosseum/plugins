...
`plan-review` 결과가 `수정 필요`이면 초안을 수정한 뒤 같은 초안 폴더를 다시 검토한다. `pass` 또는 확인 조건을 수용한 `conditional pass` 이후에만 팀의 Confluence 반영 또는 외부 공유 절차로 넘긴다. 이 플러그인은 Confluence에 직접 게시하지 않는다.

전체 문서 운영 흐름은 루트의 [`docs/diagrams/planning-confluence-document-workflow.html`](../docs/diagrams/planning-confluence-document-workflow.html), 플러그인 내부 생성·검토 흐름은 [`docs/diagrams/product-team-kit-workflow.html`](../docs/diagrams/product-team-kit-workflow.html)에서 확인한다.

...
- **Marker noise tolerance**: `plan-review`는 plan-format이 채운 마커(`[미정]`/`[가정]`/`[확인 필요]`/`[충돌 후보]`) 자체로는 결과를 낮추지 않는다. SSOT 충돌 또는 용어 어긋남이 명확할 때만 발견 사항으로 기록한다.
...
/product-team-kit:plan-format "기획 입력 또는 파일/디렉터리 경로"
    ...
    ↓ Step 3: dispatch → templates 읽기 후 main 직접 작성 → main 검증 (빈 골격·구조 일치·중복·라벨 cross-bleed) → 검증 실패 시 저장 보류, 통과 시 <outputRoot>/[안전기능명]--YYYY-MM-DD-HHMMSS/ 에 두 파일 병렬 저장
    ↓
/product-team-kit:plan-review <초안 폴더 또는 기능설계서/정책서 파일경로>
    ↓ 검토 대상·corpus 후보를 확정하고 필요한 corpus만 읽기
    ↓ main A축 SSOT 점검 + B worker(용어 일관성) 점검 → merge
    ↓ 기획팀용 리포트로 판정·먼저 할 일 출력
    ↓ 최종 결과는 수정 필요 > 조건부 통과 > 통과 순서로 보수적으로 취합
```

`plan-format`은 3-step 변환 스킬이다. config가 없거나 핵심 키 검증에 실패하면 즉시 strict-exit으로 종료하고 `set-config`를 안내한다. 이때 입력, templates, storage contract는 읽지 않는다. 입력이 부족하면 질문 루프를 만들지 않고 저장 보류를 반환한다. 이때 출력에는 부족 항목만 포함하고 보강용 입력 템플릿은 만들지 않으며, templates와 storage contract도 읽지 않는다. 디렉터리 입력은 기본 제외 경로를 적용한 뒤 하위의 모든 읽을 수 있는 UTF-8 텍스트 파일을 읽는다. 입력 크기·파일 개수 상한 없음은 의도된 계약이므로 truncate, 첫 N개 파일만 읽기, 샘플링은 금지한다. 대신 읽기 대상 텍스트 전체 확인 후 source index와 gate 근거 맵을 만들어 기능 후보·정책 후보·제외 후보를 압축하고, 읽기 대상 텍스트 전체를 확인하지 못하면 일부 근거만으로 저장하지 않고 저장 보류로 종료한다. 기존 기능설계서/정책서도 참고 입력으로 포함할 수 있지만, 저장은 새 timestamp 폴더에만 수행한다. 변환 단계에서는 templates를 본문 작성 직전에 읽고, dispatch가 기능명·역할명·용어·라벨·source index·gate 근거 맵을 고정한다. main이 파일 크기와 무관하게 기능설계서와 정책서 본문을 같은 턴에서 직접 작성한 뒤 구조 일치(기능설계서 1~8 / 정책서 1~10 섹션 헤더·admonition metadata), 표 컬럼 일치, 빈 골격, 중복 항목, 라벨 cross-bleed, marker 합산을 검증한다. 표는 컬럼명·컬럼 순서·컬럼 수를 템플릿과 동일하게 유지하고 채울 근거가 없는 셀은 marker(`[미정]`/`[확인 필요]`) 또는 `해당 없음` fill 문구로 채운다 (빈 위치 보존 원칙 — admonition 줄·표 row·셀 삭제 금지). 섹션 6 이상은 근거 전무 시 row 1의 첫 데이터 셀에 대표 fill 후 나머지 셀도 `해당 없음`으로 채워 row·섹션을 유지한다. main 검증 실패(빈 골격, 구조 불일치 retry 2회 fail) 시 저장 절차에 진입하지 않고 검증 실패 보류 분기로 전환해 저장 보류 템플릿으로 종료한다. 중복과 cross-bleed는 전체 재작성 없이 위반 문장·행·셀만 국소 repair한다. 저장 contract는 main 검증 통과 후 저장 절차 직전에만 읽으며, 두 final 파일은 단일 메시지에서 병렬 write한다 (verify·rename·종료 verify는 순차). 저장 완료 시 읽을 수 없는 파일은 `[읽기 제외 항목]`에, 읽었지만 본문 책임이 아닌 내용은 `[본문 제외 항목]`에 남긴다. 생성된 초안은 review target이지 Product Docs SSOT 근거가 아니다. `plan-format`은 Product Docs SSOT를 검색하거나 검증하지 않는다.

`plan-review`는 검토 대상과 짝문서를 확정한 뒤, 키워드로 좁힌 Product Docs Markdown과 필요한 linked local resource만 읽어 2축(SSOT 충돌, 용어 일관성)으로 점검한다. config 치명 오류나 지원하지 않는 문서 타입에서는 review rules와 SSOT corpus를 읽지 않고 종료 템플릿만 적용한다. SSOT corpus 추출에 실패하면 worker를 실행하지 않고 `수정 필요`로 종료한다. corpus 추출은 성공했지만 매칭이 0건이면 A축은 `검증 대상 없음`으로 처리하고, B축은 검토 대상 본문만으로 점검한다. 후보가 있으면 상위 20줄 인덱스 스캔으로 archive/deprecated/낮은 버전/키워드 미매칭 문서를 전문 읽기에서 제외한 뒤 핵심 후보만 읽는다. main이 A축 SSOT 충돌을 직접 점검하고, B worker(용어 일관성)는 본문 기반 발견 사항만 작성한다. 이후 main이 dedup, 보수 합성, 결과 판정, 리포트 출력을 담당한다. 병렬 worker 호출이 불가능한 환경에서는 같은 결과 형식으로 단일 패스 fallback을 사용한다. `<outputRoot>/` 하위 파일은 검토 대상으로 읽을 수 있지만 SSOT 근거로 사용하지 않는다. 마커(`[미정]`/`[가정]`/`[확인 필요]`/`[충돌 후보]`) 자체는 plan-format 책임 영역이며, plan-review는 본 검토에서 마커 자체로 결과를 낮추지 않는다. 다만 마커 본문이 SSOT와 충돌하거나 용어 어긋남에 해당하면 각 축 발견으로 기록한다. 출력 상단에는 기획팀이 바로 볼 수 있도록 판정, 한 줄 결론, 먼저 할 일, 기준 문서 충돌을 보여준다. 결과가 `수정 필요`이면 하단에 재검토 안내 체크리스트를 출력하고, 초안을 수정한 뒤 같은 폴더를 다시 검토한다. `통과` 또는 `조건부 통과`이면 발행 준비 체크리스트를 출력한다. 이 체크리스트는 외부 게시가 아니라 팀의 외부 반영 또는 공유 절차로 넘기기 전 사람이 확인할 항목이다. 출력은 사람용 markdown 리포트 하나로 통일하며 별도 YAML manifest 블록은 사용하지 않는다. 기능/화면설계서 또는 정책서가 아닌 입력은 검토 결과가 아니라 `올바른 검토 대상이 아님` 안내로 종료한다.
...
- `agents/` — plan-review B(용어 일관성) 내부 worker 정의
...
- `skills/plan-review/` — Product Docs SSOT 근거 기반 발행 전 초안 품질 검토 (2축: SSOT 충돌, 용어 일관성)
- `skills/plan-review/references/` — `review-rules.md` (판정·합성·근거 규칙), `output-format.md` (3종 결과 템플릿)
...
| `outputRoot` | `set-config`, `plan-format`, `plan-review` | `planning` | 초안 저장 root 폴더명. `plan-review` SSOT exclude에도 자동 반영 |
...
JSON을 직접 편집하지 않고 대화형으로 갱신하려면 `/product-team-kit:set-config`(Claude Code) 또는 `$set-config`(Codex)를 사용한다. `outputRoot`, `ssot.include`, `ssot.exclude` 3개 키는 한 번의 질문 묶음으로 현재 값(또는 default) 유지 / 다른 값 입력을 선택하고, 다른 값이 필요한 키만 batch로 입력한다. 저장 전 검증과 저장 확인을 통과해야 파일을 쓰며, config 저장이 성공하면 `CLAUDE.md`와 `AGENTS.md`는 선택 없이 항상 처리된다. 기존 config의 unknown key와 agent 안내 파일의 product-team-kit 관리 블록 밖 사용자 내용은 보존된다.

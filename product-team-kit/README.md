...
(선택) /product-team-kit:plan-review <파일경로>
    ↓ 병렬 fresh-context reviewer 3개로 근거·결정/범위·실행/검증 가능성 검토
    ↓ 최종 verdict는 수정 필요 > conditional pass > pass 순서로 보수적으로 취합
```

`plan-format`은 질문 루프 없이 단일 패스로 변환한다. 입력 해석과 공통 기준 확정은 순차로 처리하고, 기능설계서/정책서 본문 작성만 병렬화한다. 확인되지 않은 내용은 `[미정]`, `[가정]`, 확인 필요 질문으로 남기고, Confluence 근거 검증과 발행 전 결정·실행 가능성 판단은 `plan-review`로 확인한다. `plan-review`는 템플릿 구조 검증 도구가 아니다.

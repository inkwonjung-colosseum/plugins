...
기능설계서 초안에는 기획자가 정하는 `인수 조건 / 확인 기준`을 포함한다.
...
`plan-review`는 3개의 병렬 fresh-context reviewer가 같은 파일을 직접 읽고 관점별 verdict를 낸 뒤, 가장 보수적인 verdict를 최종 결과로 사용한다.

검토 관점:

- 근거: 기존 Confluence 문서와 충돌하는지 확인한다.
- 결정·범위: 결정해야 할 정책과 예외, 결정 주체, 후속 액션, 관련 정책서/기능설계서 trace 연결이 빠졌는지 확인한다.

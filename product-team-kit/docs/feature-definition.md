...
SSOT 탐색 전에 검토 대상 본문만으로 핵심 섹션 `[미정]` 3개 이상, 필수 섹션 2개 이상 실질 내용 없음, `[충돌 후보]` 3개 이상이면 `수정 필요 (조기 판정)`으로 종료한다. 키워드 추출이 실패하면 worker를 실행하지 않고 `수정 필요`로 종료한다. 키워드 추출은 성공했지만 SSOT corpus 매칭이 0건이면 A축은 `검증 대상 없음`으로 처리하고, B/C/D축은 검토 대상 본문만으로 점검한다. 근거 기록은 사람용 리포트 하단의 `읽은 근거`, `읽지 않은 관련 후보`, `제외 후보`, `검증 한계`로 남긴다. 핵심 근거를 확보하지 못하면 최종 결과는 보수적으로 낮춘다.

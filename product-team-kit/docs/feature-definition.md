...
| 발행 전 검토 | review | SSOT 충돌과 용어 일관성을 보수적으로 판정한다. |
...
corpus 추출이 실패하면 worker를 실행하지 않고 `수정 필요`로 종료한다. corpus 추출은 성공했지만 SSOT corpus 매칭이 0건이면 A축은 `검증 대상 없음`으로 처리하고, B축은 검토 대상 본문만으로 점검한다. 마커(`[미정]`/`[가정]`/`[확인 필요]`/`[충돌 후보]`) 자체는 plan-format 책임 영역이며, plan-review는 마커 자체로 결과를 낮추지 않는다. 다만 마커 본문이 SSOT와 충돌하거나 용어 어긋남에 해당하면 각 축 발견으로 기록한다. 근거 기록은 사람용 리포트 하단의 `읽은 근거`, `읽지 않은 관련 후보`, `제외 후보`, `검증 한계`로 남긴다. 핵심 근거를 확보하지 못하면 최종 결과는 보수적으로 낮춘다.

### 4.3 review gate

점검 실행은 dispatch → main A축 점검 + B worker(용어 일관성) 병렬 → merge 3단계다. dispatch가 검토 대상, 키워드, SSOT 후보를 고정하고 필요한 rules/corpus만 읽는다. SSOT corpus는 main이 보유하므로 A축 SSOT 충돌은 main이 직접 점검하고, B worker는 inline으로 전달된 검토 대상에서 용어 일관성 발견 사항만 작성한다. main이 모든 발견 사항을 dedup하고 보수 합성, 결과 판정, 리포트 출력을 담당한다. 병렬 worker 호출이 불가능한 환경에서는 같은 결과 형식으로 단일 패스 fallback을 사용한다.

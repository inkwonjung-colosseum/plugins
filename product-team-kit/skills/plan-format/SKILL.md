...
main이 dispatch 결과를 single source로 사용해 두 문서를 같은 턴에 직접 작성한다. 파일이 크거나 입력 단편이 많아도 기능설계서/정책서 worker로 분리하지 않는다. 큰 입력의 비용은 섹션 6 이상 tail 압축, semantic table validation, 중복/cross-bleed 국소 repair로 제어한다.

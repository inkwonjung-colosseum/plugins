...
`/product-team-kit:plan-format`으로 저장한 초안 폴더 또는 기능설계서/정책서 파일을 발행 전에 검토하는 스킬이다. 템플릿 구조 검사가 아니라 Product Docs SSOT 충돌, 명확성, 용어 일관성을 3축으로 점검하는 gate다.
...
각 축은 dispatch에서 확정한 검토 대상 본문과 필요한 SSOT 근거 패키지로 점검한다. 상세 기준은 `references/review-rules.md`의 `## 3축 점검 기준` 섹션을 단일 진실 소스로 따른다.
...
분류·고정 항목 결정 + A축 SSOT 충돌 점검을 수행한다. B/C 본문 점검은 worker가 담당한다.
...
main이 단일 어시스턴트 메시지에 Agent tool 호출 2 block 동시 발행한다. main A축 점검과 2 worker 호출은 동시에 진행 가능하다. 2 worker 호출이 모두 회신될 때까지 merge로 가지 않는다.

호출 환경에서 Agent 병렬 호출이 불가능하면 (Codex, MCP 단일 패스 환경) 단일 패스 fallback으로 main이 3축을 순차 점검한다. 결과 형식은 동일해야 한다.

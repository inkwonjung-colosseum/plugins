...
5. 근거 검토자는 검토 대상 파일을 직접 읽고, 초안에 명시된 관련 문서, tree 기준 상위/하위 문서, 원문 참조 문서, sibling 문서 중 확정 주장과 충돌 가능성이 있는 근거를 충분히 확장해 읽은 뒤 근거 패키지를 만든다. 전체 export 공간을 한 번에 읽지 않는다. 폴더 입력 또는 짝문서가 있는 단일 파일 입력이면 기능설계서와 정책서를 모두 읽는다. 다른 검토자가 추가 근거 후보를 발견하면 읽지 않은 관련 후보 또는 검증 한계로 남긴다.
   - 검토 대상 파일명, 문서 제목, 본문 메타의 `버전`, 또는 관련 Confluence 후보 문서명에 `v0.8`, `v0.9`, `v1.0` 같은 버전 표기가 있으면 version-aware 근거 필터를 적용한다.
   - version-aware 필터가 켜진 경우 버전이 명시된 Confluence 후보는 `v0.8` 이상만 검증 근거로 읽는다. `v0.8` 미만 문서는 현재 발행 검증 근거로 사용하지 않고 `읽지 않은 관련 후보`에 제외 사유를 남긴다.
   - 버전이 없는 직접 관련 문서는 읽을 수 있지만, 버전 확인 불가를 `검증 한계`에 남기고 그 근거만으로 `pass`를 반환하지 않는다.
...
- 검토 대상 또는 후보 문서명에서 버전 표기를 확인할 수 있으면 `v0.8` 이상 문서만 현재 발행 검증 근거로 사용한다
- version-aware 필터로 제외한 `v0.8` 미만 문서는 읽지 않은 관련 후보에 제외 사유와 함께 기록한다
- 버전이 없는 직접 관련 문서를 근거로 읽은 경우 버전 확인 불가를 검증 한계에 남기며, 그 근거만으로 `pass`를 반환하지 않는다
